# TODO

- check if insert pages allows multiple files to be inserted
- rearrange interface for insert pages
  - insertion page below source file
  - start and end pages below each insertion file

- rearrange file where new_pos is within moving interval
- check if streamlit gui makes correct validation for each feature

# DONE
- design interface for all actions except for extract
- comment code for extract_pages function in streamlit
- extract_pages interface finished
- modify features to work with uploaded_file objects instead of file path strings
- fix start and end validation
- remove page checking from feature scripts: streamlit will make validation